...
public class Node implements NodeTraverser<Node> {
     ...
     * 
     *         Easy to be traversed in Freemarker as freemarker takes the input data in Map structure
     * 
     *         Easy to be converted into Json
     * 
     *         Attributes is now supported by the Map Structure for all nodes except for list type nodes
     * 
    ...
    private Object buildJson(Node tmpNode) {
        if (tmpNode.getChild() != null) {
            ...
            if (childNodeList.size() == 1) {
                ...
                if (childNodeList.get(0).getAttribute().size() > 0) {
                    HashMap<String, Object> map1 = new HashMap<>();
                    addAttributesToMap(map1, childNodeList.get(0).getAttribute());
                ...
                if (childNodeList.get(0).getTagName()
                    ...
                    for (Node node : childNodeList) {
                        if (node.getAttribute().size() > 0) {
                            ...
                            map.put("\"" + node.getTagName() + "\"", buildJson(node));
                        }
                    }
                    return map;
                }
        ...
        return "\"" + tmpNode.getVal().trim() + "\"";
    }

    private HashMap<String, Object> addAttributesToMap(HashMap<String, Object> mainMap,
            HashMap<String, String> map) {

        Set<Entry<String, String>> entrySet = map.entrySet();
        for (Entry<String, String> entry : entrySet) {
            mainMap.put("\"" + entry.getKey() + "\"", "" + entry.getValue().trim() + "");
        }
        return mainMap;
    }
